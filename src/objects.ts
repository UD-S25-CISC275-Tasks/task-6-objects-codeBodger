...
export function makeBlankQuestion(
    ...
    return {id, name, body: "", type, options: [], expected: "", points: 1, published: false};
}
...
export function isCorrect(question: Question, answer: string): boolean {
    return question.expected.trim().toLocaleLowerCase() === answer.trim().toLocaleLowerCase();
}
...
export function isValid(question: Question, answer: string): boolean {
    return question.type === "multiple_choice_question" ? question.options.includes(answer) : true;
}
...
export function toMarkdown(question: Question): string {
    return `# ${question.name}\n${question.body}` + (question.type === "multiple_choice_question" ? "\n- " + question.options.join("\n- ") : "");
}
...
export function duplicateQuestion(id: number, oldQuestion: Question): Question {
    return {...oldQuestion, name: `Copy of ${oldQuestion.name}`, published: false, id, options: [...oldQuestion.options]};
}
...
export function mergeQuestion(
    ...
    return {...contentQuestion, options: [...contentQuestion.options], points, id, name, published: false};
}
